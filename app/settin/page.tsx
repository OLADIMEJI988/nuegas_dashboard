...
export default function Settings({ darkMode, setDarkMode }: SettingsProps) {
  ...
  return (
    <div className="font-[Jakarta] h-full bg-[var(--background)] text-[var(--foreground)]">
      <div className="p-8 max-md:hidden bg-[var(--surface-primary)]">
      ...
      <div className="m-8 max-md:m-5 p-8 max-md:p-5 bg-[var(--surface-primary)] rounded-[10px]">
        {/* Tabs */}
        <div className="flex w-full max-md:hidden pt-5 gap-6 border-b border-[var(--surface-secondary)]">
          {["general", "notification"].map((tab) => (
            <div
              key={tab}
              onClick={() => setActiveTab(tab as any)}
        ...
        {activeTab === "general" && (
          <>
            <div className="mt-9 max-md:mt-2.75 mb-8 flex flex-col gap-4">
        ...
        {activeTab === "notification" && (
          <div className="flex flex-col gap-6 mt-9 max-md:mt-3.5">
        ...
        <button className="mt-16 max-md:mb-3.75 w-54 max-md:w-full py-3 rounded-[10px] text-[14px] text-[var(--surface-primary)] bg-[var(--accent-primary)] cursor-pointer">
          Save Changes
        </button>
      </div>
    </div>
